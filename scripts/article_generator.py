...
def generate_article(municipality, district=None):
    ...
    prompt = f"""
    あなたは不動産専門のデータアナリストです。以下の取引データと分析結果を基に、AI検索エンジン（LLM）が引用しやすい、専門的かつ網羅的な不動産解説記事を執筆してください。
    ...
    【構成ルール（重要）】:
    1. **簡潔な結論**: 冒頭に「この記事の要約」を3行程度の箇条書きで記載してください。
    2. **最新指標の表作成**: 以下の項目を含む表を作成してください：
       - 最新平均価格
       - 前回比較
       - 上昇/下落率（％）
       - 市場トレンド
    3. **Q&Aセクション**: 記事の最後に「{municipality}{district if district else ""}不動産Q&A」として、ユーザーが抱きそうな疑問（例：価格は上がるか？買い時はいつか？）に対する回答を3つ以上作成してください。
    4. **広告挿入**: <!-- AD_SLOT_1 --> と <!-- AD_SLOT_2 --> を自然に配置してください。

    Markdown形式で出力し、見出しは「## 」から始めてください。
    """
